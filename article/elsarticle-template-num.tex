...
% As etapas principais consistem em extrair informações chaves dos currículos e posteriormente, os analisar e ranquear. Um alternativa são os testes de pré-triagem (https://www.testgorilla.com/) que também têm o propósito de reduzir a carga inicial de candidatos. Neles, os resultados são coletados e o gerente ou RH realiza uma decisão informada acerca da permanência da pessoa no processo. 

% Dado isso, o problema reside em o que fazer com essas informações, gerentes podem ter os resultados dos testes e habilidades dos candidatos, mas a medida que a quantidade de dados aumenta, as decisões se tornam morosas e propícias a erros.
...
% Para a psicologia, pois esse processo contém inúmeras variáveis, como o tempo de experiência, indicações, conhecimentos específicos, escolaridade, e a escolha de como ponderar esses fatores para realizar um recrutamento de sucesso, tanto a curto e longo prazo, não é simples. Para a computação, porque currículos são textos muitas vezes desestruturados e as formas de como analisá-lo, extrair as informações chaves e as ranquear se mantêm como desafios na área de Data Mining e Machine Learning. 

% <falar de learning to rank>

% From the approaches without a holistic view 
...
% com perguntas relacionada a presença de proficiência em algum requisito, entretanto, 


% All approaches can deal with questions regarding the presence of knowlege in some skills, however, when it comes to knowing how much or how little the candidate knows in each area the current approaches lack a solution. 
...
% By taking into consideration that knowlegde is an ordinal variable, i.e. i am an expert in water paiting, and that companies require a kowledge level for each area, questions like "how over fitted is this candidate for the job?", "does he know less than what is asked for?", "does he know stuff that is valued?", "is he a no-go?" can be asked, because we use the order and the natural weighting of ordinal variables to our advantage, instead of treating like nominal variables. Given this, the problem resides on how to compute similarity for ordinal data.
...
% Este trabalho apresenta uma medida simples de similaridade entre o candidato e vaga, onde cada conhecimento é uma variável categórica ordinal e cada nível de proficiência, uma possível classe da variável. Tem suporte a diferentes pesos de cada requisito por sua classificação no ranking, sendo uma variável ordinal, assim conseguindo distinguir entre níveis de proficiências. Introduz o conceito de mérito por requisito.
...
% Recentemente, ênfase nos processos de matching e ranqueamento de candidatos estão surgindo por causa dos avanços de aprendizado de máquina e data mining. \cite{matching-field-relevance} investigaram o problema de matching de currículos semiestruturados com vagas de empresas reais utilizando \textit{relevance models}, e mesmo com melhorias alcançadas, reconhecem que essa tarefa ainda é difícil. 
...
% focam em extrair informações sobre os níveis de proficiência dos requisitos, somente se os tem. Portanto, a pergunta se o candidato sabe mais do que o necessário não pode ser respondida, e sim somente a se ele sabe algo.
...
% \cite{improve-ranking-candidates-2009} construiu em cima do \cite{e-gen-job-processing-2007} para enfatizar e aprofundar no ranqueamento de candidatos. 

% \cite{impact-semantic-web-2005} estuda o impacto de tecnologias da web semântica em processos de recrutamento, apresentando uma forma de calcular a similaridade entre a vaga e o currículo com base em preceitos semânticos.
...
% focam em processos aparte do cálculo da similaridade para resolver questões de níveis de proficiência, anos de experiência, etc. 
...
% não é possível determinar por meio do valor final, juntando todas as proficiências de cada requisito, o fato se o candidato sabe menos, igual ou mais do que o desejado para a vaga. 

% O peso atribuído a cada requisito é sempre um dado a ser incluído pelo usuário, sendo que neste trabalho, a ordem inerente de variáveis ordinais é alavancada e utilizada para inferir um peso para cada requisito dado o nível de proficiência exigido pela empresa na vaga.


% porque não segue uma convenção de intervalo, o que é uma das propostas da medida apresentada neste projeto.

% A proposta de \cite{impact-semantic-web-2005} é a que mais se assemelha ao intuito desta pesquisa, uma forma de calcular a similaridade entre candidato e vaga em cada requisito, com a possibilidade de ponderações, comparações de níveis de proficiência, 
...
% Ao atribuir pesos o problema de valorização uniforme de requisitos é resolvido.
